...
- With networking disabled, jobs receive system read access and write access only to the chosen canonical folder. On macOS, the effective wrapper profile keeps the ancestor directories needed to reach authorized workspace roots readable, while dynamically denying unrelated sibling paths; other user folders, mounted volumes, and temporary trees remain denied unless they are inside the chosen folder.
