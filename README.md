...
New jobs default to **gpt-5.6-luna / xhigh**. The local policy ceiling also defaults to **Luna / xhigh**: `LOCAL_CODEX_MODEL_CEILING=luna` and `LOCAL_CODEX_REASONING_CEILING=xhigh`. Model ceilings are ordered `luna < terra < sol < astra` (`astra` is `gpt-6-astra`); reasoning ceilings are ordered `none < low < medium < high < xhigh < max < ultra`. Optional model IDs or aliases (`luna`, `terra`, `sol`, `astra`) and reasoning levels are checked against Codex's live model catalog, then rejected if they exceed either local ceiling. No fallback is applied. Replies inherit the thread's last settings only while those settings remain within the ceiling; an older thread above policy must explicitly switch back to an allowed model/reasoning level.
