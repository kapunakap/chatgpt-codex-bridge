...
- Git-backed new threads use a detached adapter-managed worktree by default. The original selected checkout is recorded as `sourceCwd`, while the managed worktree is the file-write boundary. `worktree: false` deliberately returns to direct-checkout behavior;
- network-disabled jobs keep the hardened Codex permission profile: writes stay inside the selected directory, mounted volumes and temporary trees are denied, `.codex` stays read-only, and common credential files plus the user's `.gitconfig` and `.zshenv` are denied. On macOS, user-home reads are intentionally broader so profile generation stays constant-size;
...
- linked worktrees require the validated Git common directory as an additional writable root because commits, refs, and objects are shared. This does not grant writes to original checkout files, but Git metadata changes can affect every checkout of that repository;
...
- automatic retention removes only paths registered under `LOCAL_CODEX_WORKTREE_ROOT`, protects active/queued worktrees, and requires a verified private snapshot bundle before deletion. Snapshot failure is non-destructive. Bundles and declared ignored-file overlays may contain repository content and therefore stay under user-only control state;
