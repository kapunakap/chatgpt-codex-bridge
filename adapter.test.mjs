...
test("native macOS sandbox blocks sibling and symlink writes, including system temp", {
  ...
  for (const [cwd, other] of [[a, b], [b, a]]) {
    ...
    const command = 'printf allowed > "$1/inside" || exit 10; if (printf blocked > "$2/outside") 2>/dev/null; then exit 11; fi; if (printf blocked > "$1/escape/escaped") 2>/dev/null; then exit 12; fi';
